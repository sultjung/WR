  ...
  function render(){
    ...
    list.innerHTML=items.map((article)=>{
      ...
      return `<article class="news-card ${selected?"selected":""}" data-key="${escapeHtml(key)}">
        ...
        <div class="card-actions">${url?`<a href="${escapeHtml(url)}" target="_blank" rel="noopener">대표 아랍어 원문</a>`:`<span class="badge disabled">원문 URL 미확보</span>`}<button data-action="translation" type="button" disabled>전체 번역 보기</button></div>${relatedSourcesHtml(article)}
      </article>`;
    }).join("");
  }
