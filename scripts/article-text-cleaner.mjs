...
const LEADING_UI_LINE = /^(?:أنت\s+هنا|الرئيسية|أخبار\s+رئيسية|الأخبار\s+الرئيسية|سياسة|السياسة|العراق|متابعة|شارك\s+القصة|حجم\s+الخط|الخط|news|politics|home|follow-up)$/iu;
...
function removeNoisyContainers(value = "") {
  const noise = "(?:share|sharing|social|social-links|tags?|keywords?|related|recommended|more-news|most-read|most-viewed|popular|sidebar|breadcrumb|breadcrumbs|newsletter|comments?|advert(?:isement)?|author-box|post-meta|article-meta)";
...
function semanticText(value = "") {
  ...
  const blocks = [];
  for (const match of html.matchAll(/<(p|h2|h3|h4|li|blockquote)\b[^>]*>([\s\S]*?)<\/\1>/gi)) {
...
export function cleanArticleText(value = "", { title = "" } = {}) {
  const lines = semanticText(value).split(/\r?\n/).map(normalizedLine).filter(Boolean);
  const kept = [];
